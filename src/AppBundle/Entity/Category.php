<?php

namespace AppBundle\Entity;

use Doctrine\Common\Collections\ArrayCollection;
use Doctrine\ORM\Mapping as ORM;

/**
 * Category
 *
 * @ORM\Table(name="category")
 * @ORM\Entity(repositoryClass="AppBundle\Repository\CategoryRepository")
 */
class Category
{
    /**
     * @var int
     *
     * @ORM\Column(name="id", type="integer")
     * @ORM\Id
     * @ORM\GeneratedValue(strategy="AUTO")
     */
    private $id;

    /**
     * @var string
     *
     * @ORM\Column(name="name", type="string", length=255)
     */
    private $name;

    /**
     * @var TypeMission
     * @ORM\OneToMany(targetEntity="AppBundle\Entity\TypeMission" ,mappedBy="category" ,cascade={"persist", "remove"})
     */
    private $typeMissions;

    /**
     * Category constructor.
     */
    public function __construct()
    {
        $this->typeMissions = new ArrayCollection();
    }


    /**
     * Get id.
     *
     * @return int
     */
    public function getId()
    {
        return $this->id;
    }

    /**
     * Set name.
     *
     * @param string $name
     *
     * @return Category
     */
    public function setName($name)
    {
        $this->name = $name;

        return $this;
    }

    /**
     * Get name.
     *
     * @return string
     */
    public function getName()
    {
        return $this->name;
    }

    /**
     * @param TypeMission $typeMission
     * @return $this
     */
    public function addTypeMission(TypeMission $typeMission)
    {
        $this->typeMissions->add($typeMission);

        return $this;
    }

    /**
     * @param TypeMission $typeMission
     * @return mixed
     */
    public function removeTypeMission(TypeMission $typeMission)
    {
        return $this->typeMissions->removeElement($typeMission);
    }

    /**
     * @return TypeMission
     */
    public function getTypeMissions(): TypeMission
    {
        return $this->typeMissions;
    }

    /**
     * @param TypeMission $typeMissions
     */
    public function setTypeMissions(TypeMission $typeMissions)
    {
        $this->typeMissions = $typeMissions;
    }


}
