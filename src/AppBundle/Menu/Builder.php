...
class Builder implements ContainerAwareInterface
{
    ...
    public function createMainMenu(array $options)
    {
        ...
        $menu->addChild('Main', [
            'label' => '<span>Main</span> <i class="icon-menu" title="Main pages"></i>',
        ...
        $menu->addChild('Utilisateurs', [
            'uri' => '#',
            'label' => '<i class="icon-users"></i> <span>Utilisateurs</span>',
            'extras' => ['safe_label' => true],
        ]);

        $menu->addChild('Contrats', [
            'uri' => '#',
            'label' => '<i class="icon-file-text3"></i> <span>Contrats</span>',
            'extras' => ['safe_label' => true],
        ]);

        $menu->addChild('Sinistres', [
            'uri' => '#',
            'label' => '<i class="icon-books"></i> <span>Sinistres</span>',
            'extras' => ['safe_label' => true],
        ]);
        $menu->addChild('Surfaces', [
            'uri' => '#',
            'label' => '<i class="icon-home5"></i> <span>Surfaces</span>',
            'extras' => ['safe_label' => true],
        ]);

        $menu->addChild('CRM', [
            'uri' => '#',
            'label' => '<i class="icon-collaboration"></i> <span>Relations Client</span>',
            'extras' => ['safe_label' => true],
        ])
        ;
        $settings = $menu->addChild('Parametrage', [
            'uri' => '#',
            'label' => '<i class="icon-cog5"></i> <span>Paramètrage</span>',
            'extras' => ['safe_label' => true],
        ]);
